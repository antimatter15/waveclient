...
class WaveChannel():
    ...
    def search(self, sid, group = "public@a.gwave.com", num = 100, start = 0):
    ...
    def process_stream(self, sid, f):
        ...
        while True:
          sect = self.process_readline(res)
          if sect is '':
            break #its over
          try:
            int(sect)
            ...
            if isinstance(innerobj["p"]["1"],list):
                for w in innerobj["p"]["1"]:
                  try:
                    ...
                    content = ""
                    for c in w["10"]:
                      content += c["1"].encode("ASCII","ignore")[0:20]
                    waveid = w["1"].encode("ASCII","ignore")
                    print waveid+" : " + ctitle + " -- "+content
                  except:
                    pass
        return
